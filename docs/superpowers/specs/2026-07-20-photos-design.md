...
- Presigned direct-to-R2 upload (client uploads bytes; backend never proxies files).
- Automated image moderation on register (OpenAI omni-moderation), **fail-closed**.
...
- A Cloudflare **R2 bucket** (public, or private origin behind a public CDN domain).
- An R2 **API token** (S3-compatible access key + secret).
- A **public base URL** for served objects (R2 public bucket URL or a Cloudflare CDN domain).
...
`games/{game_id}/{uuid4hex}.{ext}` where `ext` derives from the declared content type
(`image/jpeg`→`jpg`, `image/png`→`png`, `image/webp`→`webp`). Random component makes the raw
R2 URL unguessable during the brief window before moderation.
...
OpenAI client) so imports don't require credentials.

Interface:
- `generate_presigned_put(object_key: str, content_type: str, expires_in: int = 900) -> str`
  — presigned PUT URL, 15-minute expiry.
- `head(object_key: str) -> dict | None` — returns `{"size": int, "content_type": str}` if the
  object exists, else `None`.
- `delete(object_key: str) -> None` — delete the object (idempotent; swallow "not found").
...
- `check_image(image_url: str) -> bool` — calls OpenAI `moderations.create` with
  ...
  `[{"type": "image_url", "image_url": {"url": image_url}}]`. Returns `True` (safe) only if
  none of `sexual`, `sexual/minors`, `violence/graphic`, `hate`, `hate/threatening` are
  flagged.
...
```json
{
  ...
  "public_url": "https://cdn.example.com/games/<game_id>/<uuid>.jpg"
}
```

**Step 2 — client PUTs bytes directly to `upload_url`** (R2). Client compresses and enforces
the 5 MB cap before upload. Not a backend call.
...
4. `storage.head(object_key)` — object must exist (422 "upload not found") and `size` ≤ 5 MB
   (422 "photo too large").
5. `check_image(public_url)` — if `False`, `storage.delete(object_key)` and return 422
   ("Image violates community guidelines.").
6. Create `GamePhoto` with `position = current_count` (append at end; first photo gets 0 = cover).
7. Resync `Game.image_url` to the cover photo's `public_url`.
8. Return the created `GamePhotoRead`.
...
- `storage.delete(object_key)`, delete the row.
...
- `PhotoUploadUrlResponse` — `upload_url: str`, `object_key: str`, `public_url: str`
...
4. `POST /games/{id}/photos` with `{ object_key }`. On 200, add returned photo to the gallery
   UI. On 422, show the error and drop the photo (moderation reject or too-large).
...
Mock boto3 (presign/head/delete) and moderation — no live R2 or OpenAI in tests.

- `upload-url`: happy path returns url+key+public_url; bad content_type → 422; non-owner → 403;
  at 10 photos → 409; missing game → 404.
- `register`: happy path creates row, first photo becomes cover and syncs `image_url`;
  key-prefix mismatch → 422; object missing (`head` None) → 422; oversized (`head` size > 5 MB)
  → 422; moderation reject → object deleted + 422; count race at 10 → 409; non-owner → 403.
- `delete`: removes row + calls `storage.delete`; re-packs positions; resyncs cover; empties
  `image_url` when last photo removed; non-owner → 403.
...
- Set the five `R2_*` env vars in Railway before/at deploy. Without them, photo endpoints will
