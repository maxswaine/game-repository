...
def test_create_game_returns_409_when_similar_game_exists(client_with_auth):
    with patch(_PATCH_TARGET, return_value=FAKE_VECTOR):
        create_public_game(client_with_auth)  # existing game stored with FAKE_VECTOR embedding
...
def test_create_game_with_force_bypasses_duplicate_check(client_with_auth):
    with patch(_PATCH_TARGET, return_value=FAKE_VECTOR):
...
def test_create_game_proceeds_when_no_similar_games(client_with_auth):
...
def test_duplicate_check_skipped_when_openai_unavailable(client_with_auth):
    create_public_game(client_with_auth)  # no embedding stored (OpenAI fails in test env)
