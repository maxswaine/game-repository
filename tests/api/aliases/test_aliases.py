...
def test_admin_can_list_pending_aliases(client_with_auth, client_as_admin):
    game = create_public_game(client_with_auth)
    client_with_auth.post(f"/games/{game['id']}/aliases", json={"alias": "BS"})
...
def test_non_admin_cannot_list_aliases(client_with_auth):
    response = client_with_auth.get("/admin/aliases")
    assert response.status_code == 403


def test_admin_can_approve_alias(client_with_auth, client_as_admin):
    game = create_public_game(client_with_auth)
...
def test_admin_can_reject_alias(client_with_auth, client_as_admin):
    game = create_public_game(client_with_auth)
    suggest = client_with_auth.post(
        f"/games/{game['id']}/aliases", json={"alias": "BS"}
    )
...
def test_non_admin_cannot_patch_alias(client_with_auth):
    game = create_public_game(client_with_auth)
    suggest = client_with_auth.post(
        f"/games/{game['id']}/aliases", json={"alias": "BS"}
    )
...
def test_alias_patch_invalid_status_returns_422(client_with_auth, client_as_admin):
    game = create_public_game(client_with_auth)
    suggest = client_with_auth.post(
        f"/games/{game['id']}/aliases", json={"alias": "BS"}
    )
...
def test_approved_alias_appears_in_game_response(client_with_auth, client_as_admin):
    game = create_public_game(client_with_auth)
    suggest = client_with_auth.post(
        f"/games/{game['id']}/aliases", json={"alias": "BS"}
    )
